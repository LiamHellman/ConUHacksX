// Cleaned up, single valid component with all logic and JSX inside
...
function buildHighlightSpans(text, findings) {
  ...
  for (const f of findings) {
    ...
    if (e > s) { boundaries.add(s); boundaries.add(e); }
  }
  ...
  for (let i = 0; i < pts.length - 1; i++) {
    ...
    spans.push({ start: a, end: b, quote: text.slice(a, b), findings: active, primary: pickBest(active), _key: key });
  }
...
function makeDoc({ title, content, type }) {
  return { id: uid(), title: title || "Untitled", content: content || "", type: type || "text", results: null };
}
function normalizeSession(item) {
  if (!item) return null;
  if (item.docs && Array.isArray(item.docs)) {
    const docs = item.docs || [];
    return { ...item, activeDocId: item.activeDocId || docs[0]?.id || null };
  }
  const doc = makeDoc({ title: item.title || "Document", content: item.content || "", type: item.type || "text" });
  doc.results = item.results ?? null;
  return { id: item.id ?? Date.now(), seq: typeof item.seq === "number" ? item.seq : null, title: item.title || "Session", docs: [doc], activeDocId: doc.id };
}
...
export default function AnalysisPage() {
  ...
  const [checks, setChecks] = useState({ bias: true, fallacies: true, tactic: true });
  ...
  function createSessionWithDocs({ title = "Session", docs = [] }) {
    ...
    const session = { id: sessionId, title, docs, activeDocId: docs[0]?.id || null };
  ...
  function renameSession(sessionId, newTitle) {
    setHistory((prev) => prev.map((s) => s.id === sessionId ? { ...normalizeSession(s), title: newTitle } : s));
  }
  function addDocToSession(sessionId, doc) {
    setHistory((prev) => prev.map((s) => {
      ...
      return { ...next, docs: nextDocs, activeDocId: next.activeDocId || doc.id };
    }));
    setActiveSessionId(sessionId);
    ...
    (async () => {
      for (const file of files) {
        try {
          ...
          const doc = makeDoc({ title: file.name, content: String(text), type: "text" });
  ...
  const handleSelectFinding = (finding) => {
    if (!finding) { setSelectedFinding(null); return; }
  ...
  const handleAnalyze = async () => {
    ...
    setIsAnalyzing(true);
    try {
      const data = await analyzeText(activeDoc.content, { maxFindings: 10, temperature: 0.2 });
      setHistory((prev) => prev.map((s) => {
        if (s.id !== activeSessionId) return s;
        const next = normalizeSession(s);
        return { ...next, docs: (next.docs || []).map((d) => d.id === activeDoc.id ? { ...d, results: data } : d) };
      }));
  ...
  const enabledFindings = useMemo(() => {
    ...
    for (const [checkKey, typeSet] of Object.entries(CHECK_TO_TYPES)) {
      if (checks[checkKey]) { for (const t of typeSet) enabledTypes.add(t); }
    }
    return all.filter((f) => enabledTypes.has(f.type));
  }, [results, checks]);
  const docSpans = useMemo(() => buildHighlightSpans(documentContent, enabledFindings), [documentContent, enabledFindings]);
  useEffect(() => {
                          ...
                          <div className="flex flex-col min-w-0">
                            ...
                            {docCount > 1 && (
                              <span className="text-[10px] text-gray-500">
                                {docCount} docs{analyzedCount > 0 ? ` • ${analyzedCount} analyzed` : ""}
                              </span>
                            )}
                          </div>
                        </button>
